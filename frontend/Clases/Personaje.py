...
class Zombi(Personaje):
    ...
    def escuchaRuido(self):
        return self.escuchaRuido
    ...
    def definirHabilidades(self):
        listaHabilidades = []
        if self.nombre == "Apolo":
            #anula el sonido de una casilla
            listaHabilidades.append(MultiAtaque("// (Mayor Ataque):",10,"//"))
            
        elif self.nombre == "Troyano":
            listaHabilidades.append(Escalar("// (Escalar): ",1,"//"))
            
        else: #Si self.nombre == "Centauro":
            listaHabilidades.append(OidoAgudo(" // (Mayor sensibilidad al ruido):",10,"//"))
            
        return listaHabilidades

    def setImagen(self,ruta,imagen,objetoPY=False):
